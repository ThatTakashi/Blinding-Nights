...
*/
public class Chant_WhiteMoon extends Chant
{
	@Override
	public String ID()
	{
		return "Chant_WhiteMoon";
	}

	private final static String localizedName = CMLib.lang().L("White Moon");
	...
	private final static String localizedStaticDisplay = CMLib.lang().L("(White Moon)");
	...
	public void unInvoke()
	{
		...
		if(canBeUninvoked())
			R.showHappens(CMMsg.MSG_OK_VISUAL,L("The white moon sets."));
	...
	public int castingQuality(final MOB mob, final Physical target)
	{
		if(mob!=null)
		{
			if(!CMLib.flags().isLawful(mob))
				return Ability.QUALITY_INDIFFERENT;
			if((mob.isInCombat())&&CMLib.flags().isLawful(mob.getVictim()))
				return Ability.QUALITY_INDIFFERENT;
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		{
			mob.tell(L("This place is already under the white moon."));
			return false;
		}
		...
		if(success)
		{
			...
			if(mob.location().okMessage(mob,msg))
			{
				...
				if(msg.value()<=0)
				{
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,L("The White Moon Rises!"));
