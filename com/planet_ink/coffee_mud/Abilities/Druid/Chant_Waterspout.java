...
*/
public class Chant_Waterspout extends Chant
{
	@Override
	public String ID()
	{
		return "Chant_Waterspout";
	}

	private final static String	localizedName	= CMLib.lang().L("Waterspout");
	...
	private final static String	localizedStaticDisplay	= CMLib.lang().L("(Inside a Waterspout)");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		{
			mob.tell(mob,null,null,L("A waterspout is already here!"));
			return false;
		}
		...
		if(success)
		{
			...
			if(mob.location().okMessage(mob,msg))
			{
				...
				if(stuff.size()==0)
					mob.location().showHappens(CMMsg.MSG_OK_ACTION,L("The waterspout dissipates harmlessly."));
				else
				while(stuff.size()>0)
				{
					...
					if(O instanceof Item)
					{
						final Item I=(Item)O;
						if(R==mob.location())
							mob.location().show(mob,null,I,CMMsg.MSG_OK_ACTION,L("The waterspout picks up <O-NAME> and whisks it around."));
						else
						{
						...
						if(I.subjectToWearAndTear())
						{
							...
							if(damage>0)
								CMLib.combat().postItemDamage(mob, I, this, damage, CMMsg.TYP_WATER, null);
						}
					}
					else
					if(O instanceof MOB)
					{
						...
						final CMMsg msg3=CMClass.getMsg(mob,M,this,verbalCastMask(mob,M,auto)|CMMsg.TYP_WATER,null);
						...
						&&(mob.location().okMessage(mob,msg3)))
						{
							...
							if(R==mob.location())
								mob.location().show(M,null,null,CMMsg.MSG_OK_ACTION,L("The waterspout picks <S-NAME> up and whisks <S-HIM-HER> around."));
							else
							{
								mob.location().show(M,null,null,CMMsg.MSG_OK_ACTION,L("The waterspout picks <S-NAME> up and whisks <S-HIM-HER> away."));
							...
							CMLib.combat().postDamage(mob,M,this,damage,CMMsg.MASK_ALWAYS|CMMsg.TYP_WATER,Weapon.TYPE_BASHING,L("The waterspout <DAMAGE> <T-NAME>!"));
			...
			return beneficialWordsFizzle(mob,null,L("<S-NAME> chant(s) into the waters, but nothing happens."));
