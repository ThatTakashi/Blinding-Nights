...
*/
public class Prayer_EmpowerModestWeapon extends Prayer
{

	@Override
	public String ID()
	{
		return "Prayer_EmpowerModestWeapon";
	}

	private final static String localizedName = CMLib.lang().L("Empower Modest Weapon");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(CMLib.flags().isLawful(target)||CMLib.flags().isChaotic(target))
		{
			mob.tell(L("You can not empower that repulsive weapon."));
			return false;
		}
		...
		if(success)
		{
			...
			if(mob.location().okMessage(mob,msg))
			{
				...
				target.basePhyStats().addAmbiance("#MODERATE");
