...
*/
public class Prayer_InfuseDiscipline extends Prayer implements Deity.DeityWorshipper
{
	@Override
	public String ID()
	{
		return "Prayer_InfuseDiscipline";
	}

	private final static String localizedName = CMLib.lang().L("Infuse Discipline");
	...
	private final static String localizedStaticDisplay = CMLib.lang().L("(Infused Discipline)");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(target instanceof Room)
		{
			...
			{
				mob.tell(L("The faithless may not infuse lawfulness in a room."));
				return false;
			}
			...
			for(final Enumeration<Room> e=A.getMetroMap();e.hasMoreElements();)
			{
				...
				{
					mob.tell(L("There is already a lawful place of @x1 in this area at @x2.",deityName,R.displayText(mob)));
					return false;
				}
			}
		}
		...
		if(success)
		{
			...
			beneficialWordsFizzle(mob,target,L("<S-NAME> @x1 to infuse a lawful aura in <T-NAMESELF>, but fail(s).",prayForWord(mob)));
