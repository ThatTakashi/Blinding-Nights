...
*/
public class Prayer_EmpowerJustWeapon extends Prayer
{

	@Override
	public String ID()
	{
		return "Prayer_EmpowerJustWeapon";
	}

	private final static String localizedName = CMLib.lang().L("Empower Just Weapon");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(success)
		{
			...
			if(mob.location().okMessage(mob,msg))
			{
				...
				target.basePhyStats().addAmbiance("#LAW");
				...
				if(zappA==null)
				{
					...
					A.setMiscText("+FACTION -CHAOS -MODERATE -DEITY \"+"+deityName.toUpperCase().trim()+"\"");
					target.addNonUninvokableEffect(A);
				}
				else
				{
					if((zappA.text().indexOf("-MODERATE")<0)||(zappA.text().indexOf("-CHAOS")<0))
						zappA.setMiscText(zappA.text()+" +FACTION -CHAOS -MODERATE");
