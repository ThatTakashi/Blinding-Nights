...
*/
public class Prayer_EmpowerFoulWeapon extends Prayer
{

	@Override
	public String ID()
	{
		return "Prayer_EmpowerFoulWeapon";
	}

	private final static String localizedName = CMLib.lang().L("Empower Foul Weapon");
		...
		if(success)
		{
			...
			if(mob.location().okMessage(mob,msg))
			{
				...
				if(zappA==null)
				{
					...
					A.setMiscText("+FACTION -LAW -MODERATE -DEITY \"+"+deityName.toUpperCase().trim()+"\"");
					target.addNonUninvokableEffect(A);
				}
				else
				{
					if((zappA.text().indexOf("-MODERATE")<0)||(zappA.text().indexOf("-LAW")<0))
						zappA.setMiscText(zappA.text()+" +FACTION -LAW -MODERATE");
