...
*/
public class Prayer_InfuseModeration extends Prayer implements Deity.DeityWorshipper
{
	@Override
	public String ID()
	{
		return "Prayer_InfuseModeration";
	}

	private final static String localizedName = CMLib.lang().L("Infuse Moderation");
	...
	private final static String localizedStaticDisplay = CMLib.lang().L("(Infused Moderation)");
	...
	public void unInvoke()
	{
		...
		if(canBeUninvoked())
		{
			if(affected instanceof MOB)
				((MOB)affected).tell(L("Your infused moderation fades."));
		}
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(success)
		{
			...
			beneficialWordsFizzle(mob,target,L("<S-NAME> @x1 to infuse a moderation aura in <T-NAMESELF>, but fail(s).",prayForWord(mob)));
