...
public class Prayer_InfuseImpunity extends Prayer implements Deity.DeityWorshipper
{
	...
	public String ID()
	{
		return "Prayer_InfuseImpunity";
	}

	private final static String localizedName = CMLib.lang().L("Infuse Impunity");
	...
	private final static String localizedStaticDisplay = CMLib.lang().L("(Infused Impunity)");
	...
	public void unInvoke()
	{
		...
		if(canBeUninvoked())
		{
			if(affected instanceof MOB)
				((MOB)affected).tell(L("Your infused chaos fades."));
		}
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(target instanceof Room)
		{
			...
			{
				mob.tell(L("The faithless may not infuse chaoticness in a room."));
				return false;
			}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),
					auto?L("A chaotic aura appears around <T-NAME>."):L("^S<S-NAME> @x1 to infuse a chaotic aura around <T-NAMESELF>.^?",prayForWord(mob)));
			...
			beneficialWordsFizzle(mob,target,L("<S-NAME> @x1 to infuse a chaotic aura in <T-NAMESELF>, but fail(s).",prayForWord(mob)));
