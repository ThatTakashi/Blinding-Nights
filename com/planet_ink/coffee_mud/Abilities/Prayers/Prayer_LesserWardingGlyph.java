...
*/
public class Prayer_LesserWardingGlyph extends Prayer implements Trap
{

	@Override
	public String ID()
	{
		return "Prayer_LesserWardingGlyph";
	}

	private final static String	localizedName	= CMLib.lang().L("Lesser Warding Glyph");
	...
	private final static String	localizedStaticDisplay	= CMLib.lang().L("(Lesser Warding Glyph)");
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		if(commands.size()<2)
		{
			mob.tell(L("You must specify what container to target, what divine prayer to set, and any necessary parameters."));
			return false;
		}
		final String itemName=commands.get(0);
		@SuppressWarnings("unchecked")
		final Item target = super.getTarget(mob, mob.location(), givenTarget, new XVector<String>(itemName), Filterer.ANYTHING);
		if(target == null)
			return false;
		if((!(target instanceof Container))
		||(((Container)target).capacity()==0))
		{
			mob.tell(L("@x1 is not an appropriate container for a glyph.",target.name(mob)));
			return false;
		}
