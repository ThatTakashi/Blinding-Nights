...
*/
public class Prayer_InfuseHoliness extends Prayer implements Deity.DeityWorshipper
{
	@Override
	public String ID()
	{
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(target instanceof Room)
		{
			...
			for(final Enumeration<Room> e=A.getMetroMap();e.hasMoreElements();)
			{
				...
				{
					mob.tell(L("There is already a holy place of @x1 in this area at @x2.",deityName,R.displayText(mob)));
					return false;
				}
			}
		}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),
					auto?L("A holy aura appears around <T-NAME>."):L("^S<S-NAME> @x1 to infuse a holy aura around <T-NAMESELF>.^?",prayForWord(mob)));
