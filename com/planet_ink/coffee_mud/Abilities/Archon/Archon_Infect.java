...
/*
   Copyright 2017-2023 Bo Zimmerman
...
public class Archon_Infect extends ArchonSkill
{
	...
	public String ID()
	{
		return "Archon_Infect";
	}

	private final static String localizedName = CMLib.lang().L("Infect");
	...
	private static final String[] triggerStrings = I(new String[] { "INFECT" });
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		if(commands.size()<2)
		{
			mob.tell(L("Infect whom with what?"));
			return false;
		}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,CMMsg.MASK_MOVE|CMMsg.TYP_JUSTICE|(auto?CMMsg.MASK_ALWAYS:0),auto?L("A disease cloud surrounds <T-NAME>!"):L("^F<S-NAME> infect(s) <T-NAMESELF>.^?"));
			...
			return beneficialVisualFizzle(mob,target,L("<S-NAME> attempt(s) to infect <T-NAMESELF>, but fail(s)."));
		return success;
	}
}
