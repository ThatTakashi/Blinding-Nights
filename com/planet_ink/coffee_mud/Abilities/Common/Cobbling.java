...
/*
   Copyright 2004-2023 Tim Kassebaum
...
public class Cobbling extends EnhancedCraftingSkill implements ItemCraftor, MendingSkill
{
								 ...
								 final int asLevel, final int autoGenerate, final boolean forceLevels, final List<CraftedItem> crafted)
	{
		...
		if(commands.size()==0)
		{
			commonTelL(mob,"Make what? Enter \"cobble list\" for a list, \"cobble info <item>\", \"cobble refit <item>\" to resize, "
		...
		else
		{
			...
			final int[] pm=checkMaterialFrom(mob,commands,new int[]{
					RawMaterial.MATERIAL_METAL,RawMaterial.MATERIAL_MITHRIL,RawMaterial.MATERIAL_CLOTH,
					RawMaterial.MATERIAL_WOODEN,RawMaterial.MATERIAL_LEATHER
			});
			if(pm==null)
				return false;
			...
			if(foundRecipe==null)
			{
				commonTelL(mob,"You don't know how to make a '@x1'.  Try \"cobble list\" for a list.",recipeName);
				return false;
			}
