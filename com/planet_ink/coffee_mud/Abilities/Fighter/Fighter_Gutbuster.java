...
*/
public class Fighter_Gutbuster extends FighterSkill
{
	@Override
	public String ID()
	{
		return "Fighter_Gutbuster";
	}

	private final static String	localizedName	= CMLib.lang().L("Gutbuster");
	...
	private static final String[]	triggerStrings	= I(new String[] { "GUTBUSTER" });
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if((!auto)
		&&(!((CMLib.flags().isSitting(target)||CMLib.flags().isSleeping(target)))))
		{
			mob.tell(L("Your target must already be on the ground!"));
			return false;
		}
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,CMMsg.MSK_MALICIOUS_MOVE|CMMsg.TYP_JUSTICE|(auto?CMMsg.MASK_ALWAYS:0),
					auto?L("<T-NAME> groan(s)!"):L("^F^<FIGHT^><S-NAME> kneel(s) down at <T-NAME>!^</FIGHT^>^?"));
			CMLib.color().fixSourceFightColor(msg);
			if(mob.location().okMessage(mob,msg))
			{
				mob.location().send(mob,msg);
				if(msg.value()>0)
					return maliciousFizzle(mob,target,L("<T-NAME> dodge(s) <S-YOUPOSS> gutbusting move."));
			...
			return maliciousFizzle(mob,target,L("<S-NAME> attempt(s) to kneel down at <T-NAMESELF>, but fail(s)."));
		return success;
	}
}
