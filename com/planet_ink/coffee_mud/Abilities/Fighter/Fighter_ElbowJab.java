...
*/
public class Fighter_ElbowJab extends FighterSkill
{
	@Override
	public String ID()
	{
		return "Fighter_ElbowJab";
	}

	private final static String	localizedName	= CMLib.lang().L("Elbow Jab");
	...
	private static final String[]	triggerStrings	= I(new String[] { "ELBOWJAB" });
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(mob.charStats().getBodyPart(Race.BODY_ARM)<=0)
		{
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,CMMsg.MSK_MALICIOUS_MOVE|CMMsg.TYP_JUSTICE|(auto?CMMsg.MASK_ALWAYS:0),
					auto?L("<T-NAME> groan(s)!"):L("^F^<FIGHT^><S-NAME> elbow(s) <T-NAME>!^</FIGHT^>^?"));
			CMLib.color().fixSourceFightColor(msg);
			if(mob.location().okMessage(mob,msg))
			{
				mob.location().send(mob,msg);
				if(msg.value()>0)
					return maliciousFizzle(mob,target,L("<T-NAME> dodge(s) <S-YOUPOSS> elbow jab."));

				final Item I=CMClass.getWeapon("StdWeapon");
				I.setName(L("an elbow"));
			...
			return maliciousFizzle(mob,target,L("<S-NAME> attempt(s) to elbow <T-NAMESELF>, but fail(s)."));
		return success;
	}
}
