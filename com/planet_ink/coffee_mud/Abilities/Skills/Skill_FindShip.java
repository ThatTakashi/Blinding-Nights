...
public class Skill_FindShip extends StdAbility
{
	...
	public String ID()
	{
		return "Skill_FindShip";
	}

	private final static String	localizedName	= CMLib.lang().L("Find Ship");
	...
	private final static String	localizedStaticDisplay	= CMLib.lang().L("(finding your ship)");
	...
	public boolean tick(final Tickable ticking, final int tickID)
	{
		...
		if(tickID==Tickable.TICKID_MOB)
		{
			...
			if(nextDirection==999)
			{
				mob.tell(shipHere(mob,mob.location(),null));
				nextDirection=-2;
				unInvoke();
			}
			else
			if(nextDirection==-1)
			{
				if(shipHere(mob,mob.location(),null).length()==0)
					mob.tell(L("The trail to your ship dries up here."));
				nextDirection=-999;
				unInvoke();
			}
			else
			if(nextDirection>=0)
			{
				mob.tell(L("The way to your ship seems to continue @x1.",CMLib.directions().getDirectionName(nextDirection)));
				if(mob.isMonster())
				{
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg)
	{
		...
		&&((msg.sourceMinor()==CMMsg.TYP_LOOK)||(msg.sourceMinor()==CMMsg.TYP_EXAMINE)))
		{
			...
			{
				final String str=shipHere((MOB)affected,msg.target(),null);
				if(str.length()>0)
					((MOB)affected).tell(str);
			}
			else
			if((msg.target()!=null)
			&&(shipHere((MOB)affected,msg.target(),null).length()>0)
	...
	public String shipCheck(final MOB mob, final Item I, final Item container, final StringBuffer msg)
	{
		...
		&&(((NavigableItem)I).navBasis() == Rideable.Basis.WATER_BASED))
		{
			...
			if(((prop != null)
			&&(CMLib.law().doesHavePrivilegesWith(mob, prop))))
			{
				if((prop.getOwnerName().equalsIgnoreCase(mob.Name()))
				||(CMLib.players().playerExistsAllHosts(prop.getOwnerName())))
					msg.append(L("@x1 appears to be your ship!\n\r",I.name()));
			}
		}
		return msg.toString();
	}
	...
	public String shipHere(final MOB mob, final Environmental E, final Item container)
	{
		...
		if((E instanceof Room)&&(CMLib.flags().canBeSeenBy(E,mob)))
		{
			...
			for(int i=0;i<room.numItems();i++)
			{
				...
				shipCheck(mob,I,container,msg);
			}
		}
		else
		if((E instanceof Item)&&(CMLib.flags().canBeSeenBy(E,mob)))
		{
			shipCheck(mob,(Item)E,container,msg);
			msg.append(shipHere(mob,((Item)E).owner(),(Item)E));
		}
		return msg.toString();
	}
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		final String here=shipHere(mob,mob.location(),null);
		if(here.length()>0)
		{
			mob.tell(here);
			return true;
		...
		{
			final CMMsg msg=CMClass.getMsg(mob,null,this,CMMsg.MSG_QUIETMOVEMENT,auto?L("<S-NAME> begin(s) heading for <S-HIS-HER> ship!"):L("<S-NAME> begin(s) heading back to <S-HIS-HER> ship."));
			if(mob.location().okMessage(mob,msg))
			{
				mob.location().send(mob,msg);
				final Skill_FindShip newOne=(Skill_FindShip)this.copyOf();
			...
			beneficialVisualFizzle(mob,null,L("<S-NAME> attempt(s) to find <S-HIS-HER> ship, but fail(s)."));
