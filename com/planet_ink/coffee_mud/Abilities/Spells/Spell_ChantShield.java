...
public class Spell_ChantShield extends Spell
{
	...
	public String ID()
	{
		return "Spell_ChantShield";
	}

	private final static String localizedName = CMLib.lang().L("Chant Shield");
	...
	private final static String localizedStaticDisplay = CMLib.lang().L("(Chant Shield)");
	...
	public void unInvoke()
	{
		...
		if(canBeUninvoked())
			mob.tell(L("Your shield against chants fades."));
	...
	public boolean invoke(final MOB mob, final List<String> commands, final Physical givenTarget, final boolean auto, final int asLevel)
	{
		...
		if(success)
		{
			final CMMsg msg=CMClass.getMsg(mob,target,this,verbalCastCode(mob,target,auto),auto?L("<T-NAME> look(s) protected from chants."):L("^S<S-NAME> invoke(s) an anti-chant shield around <T-NAMESELF>.^?"));
