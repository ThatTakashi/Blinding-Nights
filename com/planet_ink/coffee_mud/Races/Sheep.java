...
public class Sheep extends StdRace
{
	...
	private final static String localizedStaticName = CMLib.lang().L("Sheep");
	...
	private final String[]	racialAbilityNames			= { "SheepSpeak" };
	...
	public String healthText(final MOB viewer, final MOB mob)
	{
		...
		if(pct<.40)
			return L("^y@x1^y has large patches of bloody matted wool.^N",mob.name(viewer));
		else
		if(pct<.50)
		...
		if(pct<.90)
			return L("^g@x1^g has some disheveled wool.^N",mob.name(viewer));
		else
		if(pct<.99)
			return L("^g@x1^g has some misplaced wool.^N",mob.name(viewer));
		else
