...
public class Rabbit extends StdRace
{
	...
	private final static String localizedStaticName = CMLib.lang().L("Rabbit");
	...
	private final String[]	racialAbilityNames			= { "RabbitSpeak" };
	...
	public String healthText(final MOB viewer, final MOB mob)
	{
		...
		if(pct<.10)
			return L("^r@x1^r is one unhappy bunny!^N",mob.name(viewer));
		else
		if(pct<.20)
	...
	public List<RawMaterial> myResources()
	{
		synchronized(resources)
		{
			if(resources.size()==0)
			{
				...
				resources.addElement(makeResource
				(L("some @x1 hair",name().toLowerCase()),RawMaterial.RESOURCE_FUR,L("@x1 hair",name().toLowerCase())));
				resources.addElement(makeResource
