...
public class AutoGold extends StdCommand
{
		...
		throws java.io.IOException
	{
		...
		if((!mob.isAttributeSet(MOB.Attrib.AUTOGOLD) && (parm.length()==0))||(parm.equalsIgnoreCase("ON")))
		{
			mob.setAttribute(MOB.Attrib.AUTOGOLD,true);
			mob.tell(L("Autogold has been turned on."));
		}
		else
		if((mob.isAttributeSet(MOB.Attrib.AUTOGOLD) && (parm.length()==0))||(parm.equalsIgnoreCase("OFF")))
		{
