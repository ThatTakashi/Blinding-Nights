...
public class HTTPReqProcessor implements HTTPFileGetter
{
	...
	public HTTPReqProcessor(final CWConfig config)
	{
		this.config=config;
	}
	 ...
	 * @throws HTTPException
	 */
	private DataBuffers generateStandardResponse(final HTTPRequest request, final HTTPStatus status, final Map<HTTPHeader,String> headers, final DataBuffers response) throws HTTPException
	{
	 ...
	 * @throws HTTPException
	 */
	private void confirmMimeType(final HTTPRequest request, final MIMEType mimeType) throws HTTPException
	{
	 ...
	 */
	@Override
	public String assembleFilePath(final HTTPRequest request)
	{
	...
	public File createFile(final HTTPRequest request, String filePath)
	{
		...
		if(request.getUrlPath().endsWith("/"))
		{
			...
			{
				final String browseCode = config.getBrowseCode(request.getHost(),request.getClientPort(),filePath);
	 ...
	 */
	private String[] generateETagMarker(final HTTPRequest request)
	{
	 ...
	 */
	private SimpleServletSession getServletSession(final HTTPRequest request, final ServletResponse servletResponse)
	{
	 ...
	 */
	private void checkIfModifiedSince(final HTTPRequest request, final DataBuffers buffers) throws HTTPException
	{
	 ...
	 */
	@Override
	public DataBuffers getFileData(final HTTPRequest request) throws HTTPException
	{
	...
	public DataBuffers generateOutput(final HTTPRequest request) throws HTTPException
	{
		...
		try
		{
			...
			final String specifiedHost = request.getHost(); // check for chunking
			int chunkedSize = 0;
